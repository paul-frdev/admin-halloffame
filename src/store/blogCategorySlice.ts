...
export const getCategoryById = createAsyncThunk<BlogCategoryState, string, { rejectValue: string }>('blog-category/get-category', async (id) => {
  try {
    const response = await BlogCategoryService.getBlogCategoryId(id);
